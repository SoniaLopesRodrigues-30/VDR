export function gerarHtmlOS(os: any, urlLogo: string, empresa: any = {}): string {
  ...
  if (os.validade) {
    const dataObj = new Date(os.validade);
    if (!isNaN(dataObj.getTime())) {
      if (os.validade.length === 10) {
...
<body>
  <div class="header">
    ...
    <!-- Coluna Direita: Identificação da OS e Dados do Cliente -->
    <div class="coluna-direita">
      <div class="titulo-os">
        <h1>ORDEM DE SERVIÇO</h1>
        <p>Nº OS: <strong>${os.id || 'N/A'}</strong></p>
      </div>
      
      <div class="info-cliente">
        <h3>Cliente</h3>
        <p><strong>Nome:</strong> ${cliente.nome || 'Não informado'}</p>
        ${os.cliente_id ? `<p><strong>ID do Cliente:</strong> #${os.cliente_id}</p>` : ''}
  ...
  <!-- Ficha Informativa: Prazos e Status -->
  <div class="dados-documento">
    <div>
      <p><strong>Status Atual da O.S.:</strong> ${os.status || 'Não informado'}</p>
    </div>
    <div style="text-align: right;">
      <p><strong>Data Limite Geral:</strong> ${dataValidade}</p>
    </div>
  </div>
  ...
  <div class="total-container">
    <div class="total-box">
      ...
      <strong>${formatarMoeda(Number(os.valor_total || 0))}</strong>
    </div>
  </div>
