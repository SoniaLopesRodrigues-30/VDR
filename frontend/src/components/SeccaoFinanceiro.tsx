...
export function SeccaoFinanceiro({ state }: { state: any }) {
  return (
    <fieldset className="section-divider">
      <legend className="section-subtitle"><CreditCard size={16} /> 6. Informações Financeiras e Cobrança</legend>
      <div className="form-row">
        <div className="form-group">
          ...
          <select value={state.formaPagamento} onChange={e => state.setFormaPagamento(e.target.value as any)} className="input-field">
            ...
            <option value="Cartão de Crédito">Cartão de Crédito</option><option value="Sem Pagamento">Sem Pagamento</option>
          </select>
        </div>
        <div className="form-group">
          <label className="form-label">Meio de Pagamento (Código Sefaz)</label>
          <input type="text" value={state.meioPagamento} onChange={e => state.setMeioPagamento(e.target.value)} placeholder="Ex: 15 - Pix" className="input-field" />
        </div>
      </div>
